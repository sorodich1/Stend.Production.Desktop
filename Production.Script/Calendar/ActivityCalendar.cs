...
{
    public class ZIPActivityCalendar
    {
        const string csActivityCalendar = "0.0.13.0.0.255";
        private bool _isChanged;

        [DefaultValue(csActivityCalendar)]
        public string LogicalName { get; set; }

        public ZIPDLMSCalendar ActiveCalendar { get; set; }

        public ZIPDLMSCalendar PassiveCalendar { get; set; }

        [JsonIgnore]
        public GXDateTime ActivateTime { get; set; }

        [XmlIgnore()]
        [JsonProperty("ActivateTime")]
        public string ActivateCTimeHS
        {
            get { return ActivateTime?.ToHex(false, false); }
            set { ActivateTime = GXDateTime.ParseHex(value); }
        }

        [JsonIgnore]
        public bool Changed { get => _isChanged; set => _isChanged = value; }
        ...
        public ZIPActivityCalendar()
        {
            //LogicalName = csActivityCalendar;
            ActiveCalendar = new ZIPDLMSCalendar();
            PassiveCalendar = new ZIPDLMSCalendar();
            ActivateTime = null;
            PassiveCalendar.OnChanged += ActivityCalendar_OnChanged;
            ActiveCalendar.OnChanged += ActivityCalendar_OnChanged;
        }

        private void ActivityCalendar_OnChanged(object sender, EventArgs e)
        {
            SetChanged();
        }
        ...
        private bool CheckValid()
        {
            return !string.IsNullOrEmpty(LogicalName) &&
                ActiveCalendar.IsValid &&
                PassiveCalendar.IsValid;
        }

        public void CopyTo(ZIPActivityCalendar target)
        {
            try
            {
                //target.ActiveCalendar.ChangingCheck(false);
                //target.PassiveCalendar.ChangingCheck(false);
                target.LogicalName = LogicalName;
                ActiveCalendar.CopyTo(target.ActiveCalendar);
                PassiveCalendar.CopyTo(target.PassiveCalendar);
                target.ActivateTime = ActivateTime;
            }
            finally
            {
                //target.ActiveCalendar.ChangingCheck();
                //target.PassiveCalendar.ChangingCheck();
                target.SetChanged();
            }
        }

        /// <summary>
        /// Перенос данных календаря активирования DLMS в текущий объект
        /// </summary>
        /// <param name="dlmsCalendar">источник копирования данных</param>
        /// <param name="markChanged">признак изменения текущего объекта</param>
        public void Assign(GXDLMSActivityCalendar dlmsCalendar, bool markChanged = true)
        {
            PassiveCalendar.OnChanged -= ActivityCalendar_OnChanged;
            ActiveCalendar.OnChanged -= ActivityCalendar_OnChanged;
            try
            {
                LogicalName = dlmsCalendar.LogicalName;
                ActivateTime = dlmsCalendar.Time;
                ActiveCalendar.Name = dlmsCalendar.CalendarNameActive;
                AssignDays(ActiveCalendar, dlmsCalendar.DayProfileTableActive);
                AssignWeeks(ActiveCalendar, dlmsCalendar.WeekProfileTableActive);
                AssignSeasons(ActiveCalendar, dlmsCalendar.SeasonProfileActive);

                PassiveCalendar.Name = dlmsCalendar.CalendarNamePassive;
                AssignDays(PassiveCalendar, dlmsCalendar.DayProfileTablePassive);
                AssignWeeks(PassiveCalendar, dlmsCalendar.WeekProfileTablePassive);
                AssignSeasons(PassiveCalendar, dlmsCalendar.SeasonProfilePassive);
            }
            finally
            {
                PassiveCalendar.OnChanged += ActivityCalendar_OnChanged;
        ...
        private void AssignDays(ZIPDLMSCalendar calendar, GXDLMSDayProfile[] dayProfile)
        {
            calendar.AssignDays(dayProfile);
        }

        private void AssignWeeks(ZIPDLMSCalendar calendar, GXDLMSWeekProfile[] weekProfile)
        {
            calendar.AssignWeeks(weekProfile);
        }

        private void AssignSeasons(ZIPDLMSCalendar calendar, GXDLMSSeasonProfile[] seasonProfile)
        {
            calendar.AssignSeasons(seasonProfile);
        }
