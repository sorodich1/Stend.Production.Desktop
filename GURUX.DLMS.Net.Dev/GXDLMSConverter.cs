...
namespace Gurux.DLMS
{
    ...
    public class GXDLMSConverter
    {
        ...
        private static void UpdateOBISCodeInfo(GXStandardObisCodeCollection codes, GXDLMSObject it, Standard standard)
        {
            ...
            //if (it.LogicalName == "0.0.1.0.1.255")
            //    code = list[0];
            //else 
            code = list[0];
            ...
            if (string.IsNullOrEmpty(it.Description))
            {
                it.Description =
#if ZIP
                    code.Description.Replace(" (0 суммарно)", "").Replace(" Тариф 0", " (сумма по тарифам)");
#else 
                    code.Description;
#endif
        ...
        public static DataType GetDLMSDataType(object value)
        {
            if (value == null)
            {
        ...
        public static byte[] GetBytes(object value, DataType type)
        {
        ...
        /// <returns>Converted value. If null You have to do it yourself.</returns>
        public static object ChangeFromDLMSType(object value, DataType type, bool isHex)
        {
            ...
            try
            {
                ...
                if (value is GXDateTime)
                {
                    ...
                    {
                        isHex = true;
                        value = (value as GXDateTime).DLMSValue;
                    }
                }
                else
                    if (value is byte[])
                {
                    isHex = true;
                    value = HexUtils.BytesToHexStr(value as byte[]);
                    if (type == DataType.OctetString)
                        return value;
                }
#endif
...
#if ZIP
                        { typeof(GXDateTime),  s => new GXDateTime(s) },
                        { typeof(byte[]),  s => HexUtils.HexStrToBytes(s) },
#endif
                    };
                ...
                if (value is IConvertible)
                {
                    if (t == typeof(byte[])) {
                        if (value is string)
                            ...
                            return Utils.ObjectToByteArray(value);
                    }
                    if (value is DateTime && t == typeof(GXDateTime))
                        return
                            new GXDateTime((DateTime)value);
        ...
        public static object ChangeType(object value, DataType type, CultureInfo cultureInfo)
        {
            object ret = null;
            if (type == DataType.OctetString)
            {
            ...
            {
                if (type == DataType.Structure || type == DataType.Array)
                {
