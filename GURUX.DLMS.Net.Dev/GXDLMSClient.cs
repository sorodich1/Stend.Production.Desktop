...
namespace Gurux.DLMS
{
    ...
    {
        private const string csIllegalCmd = "Недопустимая команда.";
        private const string csIllegalMethod = "Этот метод можно использовать только для создания пользовательских кадров HDLC";
        ...
        // sem 13.01.2022 private 
        public static Dictionary<ObjectType, Type> AvailableObjectTypes = new Dictionary<ObjectType, Type>();
        /// <summary>
        /// Standard OBIS code
        /// </summary>
        ...
        {
            Settings.NegotiatedConformance = 0;
            Settings.ResetBlockIndex();
        ...
        /// <summary>
        /// Generates a Запрос на освобождение.
        /// </summary>
        /// <returns>Запрос на освобождение, as byte array.</returns>
        ...
        public byte[][] Method(object name, ObjectType objectType, int index, Object value, DataType type)
        {
            ...
            {
                throw new ArgumentOutOfRangeException("Недопустимый параметр");
            }
            Settings.ResetBlockIndex();
            ...
            int index)
        {
            ...
            {
                throw new GXDLMSException("Недопустимый параметр");
            }
        ...
        private byte[][] Write2(object name, object value, DataType type, ObjectType objectType, int index)
        {
            ...
            if (type == DataType.None && value != null)
            {
                ...
                if (type == DataType.None)
                {
                    throw new ArgumentException("Недопустимый параметр. Неизвестный тип значения.");
                }
            }
            GXByteBuffer attributeDescriptor = new GXByteBuffer();
            GXByteBuffer data = new GXByteBuffer();
        ...
        public byte[][] ReadList(List<KeyValuePair<GXDLMSObject, int>> list)
        {
            ...
            {
                throw new ArgumentOutOfRangeException("Счетчик не поддерживает считывание нескольких объектов одним запросом.");
            }
            ...
            if (this.UseLogicalNameReferencing)
            {
                ...
                foreach (KeyValuePair<GXDLMSObject, int> it in list)
                {
                    ...
                    if (items.Length != 6)
                    {
                        throw new ArgumentException("Недопустимое логическое имя.");
                    }
        ...
        public byte[][] WriteList(List<KeyValuePair<GXDLMSObject, int>> list)
        {
            if ((NegotiatedConformance & Conformance.MultipleReferences) == 0)
            {
                throw new ArgumentOutOfRangeException("Счетчик не поддерживает запись нескольких объектов одним запросом.");
            }
            if (list == null || list.Count == 0)
            {
                throw new ArgumentOutOfRangeException("Недопустимый параметр.");
                                        ...
                                        List<GXKeyValuePair<GXDLMSObject, GXDLMSCaptureObject>> columns)
        {
            ...
            if (columns != null && columns.Count != 0)
            {
                ...
                {
                    throw new Exception("Сначала считайте объекты захвата.");
                }
                ...
                foreach (GXKeyValuePair<GXDLMSObject, GXDLMSCaptureObject> c in columns)
                {
                    ...
                    if (!found)
                    {
                        throw new Exception("Недопустимая колонка: " + c.Key.LogicalName);
                    }
                }
            }
        ...
        public byte[][] ReadRowsByEntry(GXDLMSProfileGeneric pg, UInt32 index, UInt32 count, int columnStart, int columnEnd)
        {
            ...
            {
                throw new Exception("Объекты захвата не считаны.");
            }
            pg.Buffer.Clear();
            Settings.ResetBlockIndex();
        ...
        public static int GetServerAddress(int logicalAddress, int physicalAddress, int addressSize)
        {
            ...
            {
                throw new ArgumentException("Недопустимый логический или физический адрес.");
            }
            return value;
        }
        ...
        public byte[][] AccessRequest(DateTime time, List<GXDLMSAccessItem> list)
        {
            ...
            foreach (GXDLMSAccessItem it in list)
            {
                ...
                {
                    throw new ArgumentOutOfRangeException(csIllegalCmd);
                }
            }
        ...
        public object ParseReport(GXReplyData reply, List<KeyValuePair<GXDLMSObject, int>> list)
        {
            ...
            {
                throw new ArgumentException(csIllegalCmd + " " + reply.Command);
            }
        }
        ...
        public byte[] CustomHdlcFrameRequest(byte command, GXByteBuffer data)
        {
            if (!GXDLMS.UseHdlc(Settings.InterfaceType))
            {
                throw new Exception(csIllegalMethod);
            }
        ...
        public byte[][] CustomFrameRequest(Command command, GXByteBuffer data)
        {
            ...
            throw new Exception(csIllegalMethod + " или WRAPPER");
        }
