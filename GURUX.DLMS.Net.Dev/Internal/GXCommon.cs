...
namespace Gurux.DLMS.Internal
{
    ...
    internal class GXCommon
    {
        ...
        public static bool TryByteParse(string stringToConvert, NumberStyles styles)
        {
            Byte byteValue;
            return Byte.TryParse(stringToConvert, styles, null, out byteValue);
        }
...
#if ZIP
                // Skip week day?
                byte dayOfWeek = buff.GetUInt8();
                value.DayOfWeek = (int)dayOfWeek;
                if (dayOfWeek == 0xFF)
                {
        ...
        private static object GetDateTime(GXDLMSSettings settings, GXByteBuffer buff, GXDataInfo info)
        {
...
#if ZIP
                isInvalid = ((dt.Status & ClockStatus.InvalidClockStatus) > 0)
                ? " (НС)"
                : ((dt.Status & ClockStatus.InvalidValue) > 0)
                   ? " (НЗ)" : "";
#endif
            ...
            catch (Exception ex)
            {
                ...
                {
#if ZIP
                    throw new Exception($"{GXCommon.ToHex(buff.Data, false, 0, 12) + isInvalid}: {ex.Message}");
#else
                    throw ex;
#endif
                }
        ...
        private static void SetOctetString(GXByteBuffer buff, object value)
        {
            if (value is string)
            {
                byte[] tmp =
                    //#if ZIP
                    //                    HexUtils.IsHexString((string)value)
                    //                        ? GXCommon.HexToBytes((string)value)
        ...
        public static DataType GetDLMSDataType(Type type)
        {
            ...
            {
#if ZIP
                throw new Exception($"Failed to convert data type {type.Name} to DLMS data type. Unknown data type.");
#else
