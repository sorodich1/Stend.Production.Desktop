    ...
    public class GXDLMSTranslator
    {
        ...
        private void CheckFrame(byte frame, GXDLMSTranslatorStructure xml)
        {
            ...
            else if ((frame & (byte)HdlcFrameType.Sframe) == (byte)HdlcFrameType.Sframe)
            {
                ...
                {
                    ++RAck;
                    xml.AppendComment($"sending={sending},N(R)={receiveSequence},N(S)={sendSequence},RAck={RAck}");

                }
                else if (sending && ((RSendSequence + 1) % 8) == receiveSequence)
                {
                    ++SAck;
                    xml.AppendComment($"sending={sending},N(R)={receiveSequence},N(S)={sendSequence},SAck={SAck}");
                }
                else if (SSendSequence != 0 && RSendSequence != 0)
                {
//#if !ZIP
                    byte expected = 0;
                    xml.AppendComment($"Invalid I Frame: {frame.ToString("X")}. Expected: {expected.ToString("X")} [N(R)=N(S)=0]");
//#endif
                }
            }
            //Handle U-frame.
            ...
            {
                if (
                    frame == 0x10 && SReceiveSequence == 0 && SSendSequence == 0
#if ZIP
                    && sending
#endif
                    )
                ...
                if ( 
                    frame == 0x30 && RReceiveSequence == 0 && RSendSequence == 0
#if ZIP
                    && !sending
#endif
                   )
                ...
                else
                {
                    ...
                    byte receiveSequence = (byte)((frame >> 5) & 7);
                    // sem 01.12.2021
                    xml.AppendComment($"I frame: Sending={sending},N(R)={receiveSequence},N(S)={sendSequence}");
                    // sem 01.12.2021 end
                    if (sending)
                    {
                        //If send.
...
//#if !ZIP
                            ...
                            byte exSsend = (byte)((expected >> 1) & 7);
                            byte exReceive = (byte)((expected >> 5) & 7);
                            xml.AppendComment($"Invalid I Frame: {frame.ToString("X")}. Expected: {expected.ToString("X")} [N(R)={exReceive},N(S)={exSsend}]");
//#endif
                        }
                    }
                    else
                    ...
                    if (GXDLMS.GetData(settings, msg.Message, data, null, null))
                    {
                        ...
                        if (!PduOnly)
                        {
                            ...
                            xml.AppendLine("<FrameType Value=\"" + data.FrameId.ToString("X") +
                                //$" RR={RReceiveSequence},SS={SSendSequence},SR={SReceiveSequence},RS={RSendSequence}" +  
                                "\" />");
            ...
            else
            {
                switch (tag)
                {
                    ...
                    case (int)(Command.ReadRequest) << 8 | (byte)VariableAccessSpecification.BlockNumberAccess:
                        ...
                        ++s.count;
                        break;
                    case (int)(Command.MethodRequest) << 8 | (byte)ActionRequestType.Normal:
                        s.requestType = (byte)(tag & 0xFF);
                        break;
                    case (int)(Command.MethodRequest) << 8 | (byte)ActionRequestType.NextBlock:
                        s.requestType = (byte)(tag & 0xFF);
                        break;
                    case (int)(Command.MethodRequest) << 8 | (byte)ActionRequestType.WithList:
                        s.requestType = (byte)(tag & 0xFF);
                        break;
                    case (int)(Command.MethodResponse) << 8 | (byte)ActionRequestType.Normal:
