...
namespace Gurux.DLMS.Objects
{
    ...
    public class GXDLMSObject
    {
        ...
        public virtual bool IsRead(int index)
        {
            if (!CanRead(index))
            {
                return true;
        ...
        {
            AccessMode acc = GetAccess(index);
            return !
                (
                  acc == AccessMode.NoAccess ||
                  acc == AccessMode.Write ||
                  acc == AccessMode.AuthenticatedWrite
                );
        }

        // sem 25.06.2021
        /// <summary>
        /// Is attribute of the object writable.
        /// </summary>
        /// <param name="index">Attribute index of the object.</param>
        /// <returns>True, if attribute of the object is writable.</returns>
        public bool CanWrite(int index)
        {
            AccessMode accessMode = GetAccess(index);
            return
                accessMode == AccessMode.Write ||
                accessMode == AccessMode.ReadWrite ||
                accessMode == AccessMode.AuthenticatedWrite ||
                accessMode == AccessMode.AuthenticatedReadWrite;
        }

        // sem 09.09.2021
        /// <summary>
        /// Is attribute of the object writable only.
        /// </summary>
        /// <param name="index">Attribute index of the object.</param>
        /// <returns>True, if attribute of the object is writable only.</returns>
        public bool CanWriteOnly(int index)
        {
            AccessMode accessMode = GetAccess(index);
            return
                accessMode == AccessMode.Write ||
        ...
        {
            const string csIllegalLN = "Invalid Logical Name.";
            ...
            {
                if (
                    !(GXCommon.TryByteParse(item, NumberStyles.HexNumber) ||
                      GXCommon.TryByteParse(item, NumberStyles.Integer))
                   )
                    throw new GXDLMSException(csIllegalLN);
            }
        }
        ...
        {
#if ZIP
            return ((ShortName != 0) ? ShortName.ToString() : LogicalName) + $" [{Description}]";
#else
            if (ShortName != 0)
            {
...
#if ZIP
        /// <summary>
        /// Update dirty state error of attribute index.
        /// </summary>
        /// <param name="attributeIndex"></param>
        /// <param name="value"></param>
        [EditorBrowsable(EditorBrowsableState.Never)]
        public void UpdateObjectError(int attributeIndex, Exception ex)
        {
            lock (Status)
            {
                if (!Status.ContainsKey(attributeIndex))
                ...
                {
                    Status[attributeIndex].Error = ex;
                }
            }
            OnChange?.Invoke(this, true, attributeIndex, null);
        }
#endif
        /// <summary>
